...
class TestMastermind < Test::Unit::TestCase
  ...
  def test_codemaker
    ...
    assert_equal({:white => 0, :black => 1}, @codemaker.evaluate([1, 1, 1, 1, 1, 1]))
    assert_equal({:white => 1, :black => 1}, @codemaker.evaluate([1, 6, 6, 6, 6, 1]))
    assert_equal(true, @codemaker.white_hit?(1, 1))
    assert_equal(false, @codemaker.white_hit?(7, 1))
