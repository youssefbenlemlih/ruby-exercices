...
class Clazz
  ...
  def Clazz.singleton_method_added(id)
    puts "Adding #{id.id2name} to Clazz"
  end
  def initialize(name)
    @name = name
  end
...
class ClazzSingelton include Singleton
  attr_reader :name
end

a = Clazz.new('tom')
a2 = Clazz.new('tom2')
a.define_singleton_method(:my_singlton_method) do
...
puts a==b
puts c==b
a.my_singlton_method
# a2.my_singlton_method # doesnt work
#b.my_singlton_method # doesnt work
